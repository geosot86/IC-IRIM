...
% Updated code: Georgia Sotiropoulou, EPFL-MISU
%updated with Phillips et al. (2017a) parameterization  and Mizuno et al.
%(1990) kernel correction
...
function [Q,soln,sol,velg,velG,INPt,enhancement] = SIM_35th(etabr,etaagg,etaRSg,etaRSG,...
    etaCOA,etaSH,duration,step,CCN,uz,Tstart,tau,BRphil)
   ...
   % set physical constants
    ...
    Nccn = CCN*10^6;    % coefficient in Twomey CCN spec: continental case HUCM 
                    % (see COSMO src_twomom line 17060)
    kccn = 0.30;     % exponent in Twomey CCN spec: continental case HUCM 
    ...
    coal2 = 0.05;   % what percentage of the large droplets coalesce
    INPstop=0.11*1000;
    ...
    init(1)  = 98000;           % pressure [Pa]
    init(4)  = Tstart;          % temperature [K]
    init(5)  = 10^(-8);         % supersaturation wrt liquid water [-]
    init(6)  = 3.2*10^(-3);         % vapor mixing ratio [-]
    init(7)  = 1*10^(-6);         % small droplet radius [m]
    ...
    Nd       = 0.01*10^(6);               % small droplet number [m-3]
    tt=0;
    ...
    mice    = rhoi*Ni*4/3*pi*ri0^3; %%%kg/m-3
    ...
    INPt     = struct('x',zeros(1,18000),'y',zeros(1,18000));
    Nice     = zeros(1,18000);
    Qice     = zeros(1,18000);
    Ndrop    = zeros(1,18000);
    counter3 = 1;
    ...
    soln    = struct('x',zeros(1,18000),'y',zeros(12,18000)); 
    counter = 1;
    ...
    sol = dde23(@microphysics,lags,[0.07*10^(-3); 0; 0; Nd; 0.0*10^(6); 0.0*10^(6); T0; sw],[0,timepts(2)]);
    ...
    sol2    = ode23s(@thermodynamics,[0,timepts(2)],init);
    l       = length(sol2.x);
    init(:) = sol2.y(:,end); 
...
%         end
        ...
        init(3) = mice; %/(1 + mliq); 
        init(2) = mliq; %/(1 + mice);
         Q.ice(tt+1)=rhoi*Ni*4/3*pi*ri0^3;
         Q.grau(tt+1) = rhodep*Ng*4/3*pi*ag0^(iG(T0)+2);
          Q.snow(tt+1)= rhodep*NG*4/3*pi*aG0^(iG(T0)+2);
           Q.ice2(tt+1)=rhoi*4/3*pi*ri0^3;
         Q.grau2(tt+1) = rhodep*4/3*pi*ag0^(iG(T0)+2);
          Q.snow2(tt+1)= rhodep*4/3*pi*aG0^(iG(T0)+2);
          
        % solve thermodynamics
        sol2    = ode23s(@thermodynamics,[t0,tf],init);
        l       = length(sol2.x);
        init(:) = sol2.y(:,end); 
       ...
       velg(tt+1)= termVel(ag0,T0, rhodep) ;
       velG(tt+1) = termVel(aG0,T0, rhodep) ;
   ...
   % remove trailing zeros from INPt
    indx   = find(INPt.y(1,:) ~= 0 & ~isnan(INPt.y(1,:)), 1, 'last'); 
    INPt.y = INPt.y(:,1:indx)./10^3; % units are [L-1]
    indx   = find(INPt.x(1,:) ~= 0 & ~isnan(INPt.x(1,:)), 1, 'last');
    ...
    num=find(~isnan(Nice));
    enhancement = Nice(num(end))/INPt.y(num(end));
   ...
   % indx   = find(Ndrop ~= 0, 1, 'last');
    ...
    function dy = microphysics(t, y, Z)
        ...
        mg= rhodep*(4/3*pi*ag0^(iG(T0)+2));
        mG= rhodep*(4/3*pi*aG0^(iG(T0)+2));
        
        if BRphil==1
        %Phillips parameterization
        Ncoll=Nphil(mg,mG,vtg,vtG,2*axis(ag0,T0),2*ag0,2*aG0);
       ...
       if   (aG0-ag0<0.01*ag0) 
       Kcoll = pi*(xig*ag0^2 + xiG*aG0^2)* sqrt(1.7*(vtG-vtg)^2+ 0.3*vtG*vtg); % Mizuno et al. (1990)
       else
       Kcoll = pi*(xig*ag0^2 + xiG*aG0^2)*(vtG-vtg);
        ...
        if (set3 == false)
            dy(1) = nucl(y(7),y(1))*imm*heaviside(tauNUC-t) + ...
                etabr*Kcoll*Ncoll*y(2)*y(3) + ...
                etaRSg*KRSg*NRS*y(2)*y(6) + ...
                etaRSG*KRSG*NRS*y(3)*y(6) + ...
                etaSH*pSH*pFR*NSH*y(6) - ...
                nucl(ylag1(7),ylag1(1))*imm*heaviside(t-taui)*heaviside(tauNUC-t+taui) - ...
        ...
        if (set4 == false)
            dy(2) = nucl(ylag1(7),ylag1(1))*imm*heaviside(t-taui)*heaviside(tauNUC-t+taui) + ... 
                etabr*Kcoll*Ncoll*ylag1(2)*ylag1(3) + ...
                etaRSg*KRSg*NRS*ylag1(2)*ylag1(6) + ...
                etaRSG*KRSG*NRS*ylag1(3)*ylag1(6) + ...
                etaSH*pSH*pFR*NSH*ylag1(6) - ...
                nucl(ylag2(7),ylag2(1))*imm*heaviside(t-taui-taug)*heaviside(tauNUC-t+taui+taug) - ... 
        ...
        dy(3) = nucl(ylag2(7),ylag2(1))*imm*heaviside(t-taui-taug)*heaviside(tauNUC-t+taui+taug) + ... 
            etabr*Kcoll*Ncoll*ylag2(2)*ylag2(3) + ...
            etaRSg*KRSg*NRS*ylag2(2)*ylag2(6) + ...
            etaRSG*KRSG*NRS*ylag2(3)*ylag2(6) + ...
            etaSH*pSH*pFR*NSH*ylag2(6) + ... 
            etaagg*Kagg*y(1)*y(2) - ...
            nucl(ylag3(7),ylag3(1))*imm*heaviside(t-taui-taug-tauf)*heaviside(tauNUC-t+taui+taug+tauf) - ... 
        ...
        cont.NU = [cont.NU nucl(T0,y(1))*imm*heaviside(tauNUC-t)]; 
    ...
    function frag = Neject(T)
        F = 2.8; Tmin = 252; gamma = 5;
        frag = F*(T - Tmin)^(1.2)*exp(-(T - Tmin)/gamma);
        frag(frag < 0) = 0; %10
    end


    function frag = Nphil(m1,m2,u1,u2,dgs,dg,dG)
      pssi=0.4;
      diamtr=dg;
...
%       
    ...
    %graupel-snow
       AF =1.58 * 1.e7 * (1+100*pssi.^2)*(1+1.33*1.e-4./dgs.^1.5);
       gam = 0.5-0.25*pssi;
     psy = 3.5*1.e-3;
       CC = 7.08*1.e6*psy;
...
%        
%        graupel-graupel
%        alpha0= 3.78*10^4*(1+0.0079/dg^1.5);
%        AF=alpha0+max(2*alpha0/3-alpha0/9*abs(T0-(273.15-15)),0);
%        gam = 0.3;
%          psy = 3.5*1.e-3;
%        CC = 6.3*1.e6*psy;
%      
       ...
       area=pi*dgs.^2;
       %if abs(vtG-vtg)>0.0
      %  Ko=0.5*(m1*m2)./(m1+m2).*(sqrt((1.2*vtG-0.95*vtg)^2+0.08*vtG*vtg)^2;
     if (dG-dg<0.01*dg)  %%
       Ko=0.5*(m1*m2)./(m1+m2).*(1.7*(u1-u2)^2+ 0.3*u1*u2);
     else
       Ko=0.5*(m1*m2)./(m1+m2).*(u1-u2)^2;  
    end
      frag = area.*AF.*(1-exp(-((CC*Ko)/(area*AF)).^gam));
    end
    ...
    function newNi = nucl(T0,Ni)
        ...
        if (T0 > 237 && T0 < 273)
           aa = -14.625; 
            ...
            lapse = -6/1000;
                % moist adiabatic lapse rate of 6 K/km -> [K m-1]
         %   newNi = 0.08*1000/3;%aa*uz*lapse*exp(-0.125*(T0 - 273.2))/f;
  %          newNi = aa*uz*lapse*exp(-0.125*(T0 - 273.2));
 %          a=0.0000594; b=3.33; c=0.0264; d=0.0033;
  %           aero05=0.26; 
%             aero05_std=(273/T0)*(980/1000)*aero05;
 %            newNi= a*(273.15-T0).^b*aero05_std.^(c*(273.15-T0)+d);
%             newNi=1/((273/T0)*(980/1000))*newNi*1000;
              naero_cm3=0.6;
     naero_std = (273.15/T0)*(100000/P0)* naero_cm3;
     
 %    INPstd = 0.0000594*(273.15-T0)^3.33*(naero_std)^(0.0264*(273.16-T0)+0.0033);

 INPstd =uz*lapse*( -3.33*0.0000594*(273.15-T0)^2.33*(naero_std)^(0.0264*(273.16-T0)+0.0033)-...
  0.0000594*(273.15-T0)^3.33 * 0.0264*log(naero_std)*(naero_std)^(0.0264*(273.16-T0)+0.0033));

      newNi= 1/(273.15/T0)/(100000/P0)*INPstd*1000*5;
%
% 
%             newNi=newNi-Ni;
%              newNi(newNi<=0)=0;
